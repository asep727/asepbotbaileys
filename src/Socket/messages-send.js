...
const makeMessagesSocket = config => {
    ...
    const asepBotInteractive = new interactive_handler_1.AsepBotInteractiveHandler(waUploadToServer, relayMessage, config, sock);
    ...
    return {
        ...
        sendStatusMention: async (content, jids = []) => {
            return await asepBotInteractive.sendStatusWhatsApp(content, jids);
        },
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            else if (asepBotInteractive.detectType(content)) {
                const { quoted } = options;
                const messageType = asepBotInteractive.detectType(content);
                switch (messageType) {
                    case 'PAYMENT': {
                        const paymentContent = await asepBotInteractive.handlePayment(content, quoted);
                        return await relayMessage(jid, paymentContent, {
                            messageId: (0, Utils_1.generateMessageIDV2)(userJid)
                        });
                    }
                    case 'PRODUCT': {
                        const productContent = await asepBotInteractive.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE': {
                        const interactiveContent = await asepBotInteractive.handleInteractive(content, jid, quoted);
                    ...
                    case 'INTERACTIVE_BUTTONS': {
                        const ibContent = await asepBotInteractive.handleInteractiveButtons(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        return await asepBotInteractive.handleAlbum(content, jid, quoted);
                    case 'EVENT':
                        return await asepBotInteractive.handleEvent(content, jid, quoted);
                    case 'POLL_RESULT':
                        return await asepBotInteractive.handlePollResult(content, jid, quoted);
                    case 'GROUP_STORY':
                        return await asepBotInteractive.handleGroupStory(content, jid, quoted);
                }
            }
            else {
                let mediaHandle;
