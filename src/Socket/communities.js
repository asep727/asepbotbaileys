...
const makeCommunitiesSocket = config => {
    ...
    return {
        ...
        communityParticipantCount: jid => mexQuery({ group_input: { group_id: jid } }, COMMUNITY_MEX_QUERY_IDS.QUERY_PARTICIPANT_COUNT, 'xwa2_group_query_by_id'),
        communitySubgroupsMex: jid => mexQuery({ group_input: { group_id: jid } }, COMMUNITY_MEX_QUERY_IDS.QUERY_SUBGROUPS, 'xwa2_group_query_by_id'),
        communitySubgroupParticipantCount: subgroupJid => mexQuery({ group_input: { group_id: subgroupJid } }, COMMUNITY_MEX_QUERY_IDS.QUERY_SUBGROUP_PARTICIPANT_COUNT, 'xwa2_group_query_by_id'),
        communityTransferOwnershipMex: (communityJid, newOwnerJid) => mexQuery({
            group_id: communityJid,
            role_updates: [{ user_jid: newOwnerJid, new_role: 'SUPERADMIN_MEMBER' }]
        }, COMMUNITY_MEX_QUERY_IDS.UPDATE_OWNER, 'xwa2_community_update_owner'),
