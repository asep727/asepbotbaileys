...
const makeBusinessSocket = config => {
    ...
    const getBusinessEligibility = async (features = {}) => {
        const attrs = {};
        if (features.metaVerified !== undefined)
            attrs.meta_verified = String(features.metaVerified);
        if (features.marketingMessages !== undefined)
            attrs.marketing_messages = String(features.marketingMessages);
        if (features.genai !== undefined)
            attrs.genai = String(features.genai);
        const result = await query({
            tag: 'iq',
